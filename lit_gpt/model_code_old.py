...
class GPT(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.cls_head = nn.Linear(config.n_embd,2)
    ...
    def forward(
        ...
        cos, sin = self.build_rope(idx,positions)

        if use_kv_cache:
            # cos = cos.index_select(0, input_pos)
            # sin = sin.index_select(0, input_pos)
            cos = cos.index_select(1, input_pos)
            sin = sin.index_select(1, input_pos)
        ...
        if classify:
            sequence_lengths = (torch.eq(idx, 0).int().argmax(-1) - 1).to(
                x.device
            )
            
            last_token_hidden_states = x[torch.arange(B, device=x.device), sequence_lengths]
            return self.cls_head(last_token_hidden_states)  # (B,2)
        elif last_hidden:
            # Calculate average of entire sequence embedding instead of last token
            # Create mask to exclude padding tokens (where idx == 0)
            mask = (idx != 0).float().unsqueeze(-1)  # (B, T, 1)
            # Apply mask and calculate mean along sequence dimension
            masked_x = x * mask  # (B, T, embed_size)
            sum_embeddings = masked_x.sum(dim=1)  # (B, embed_size)
            sequence_lengths = mask.sum(dim=1)  # (B, 1)
            # Avoid division by zero
            sequence_lengths = torch.clamp(sequence_lengths, min=1.0)
            average_embedding = sum_embeddings / sequence_lengths  # (B, embed_size)
            return average_embedding    # (B,embed_size)
        else:
    ...
    def build_rope(self,idx:torch.Tensor,seq_pos:torch.Tensor) -> RoPE:
        return build_rope(
            seq_pos=seq_pos,
            n_elem=int(self.config.rotary_percentage * self.config.head_size),
            dtype=torch.get_default_dtype(),
...
def build_rope(
    ...
    # seq_pos should be (batch_size, seq_len) representing position indices for each sequence in the batch
    # Normalize seq_pos based on the condense_ratio
    seq_idx = seq_pos / condense_ratio  # Shape: (batch_size, seq_len)
    # Calculate the product of position index and theta
    # We use broadcasting to compute the outer product for each batch sequence.
    # seq_idx: (batch_size, seq_len), theta: (n_elem / 2), result: (batch_size, seq_len, n_elem / 2)
    idx_theta = torch.einsum('bs,d->bsd', seq_idx, theta).repeat(1, 1, 2)
...
def apply_rope(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
    ...
    # Assume cos and sin are computed based on seq_pos and theta, and are of shape (B, T, hs)
    # Step 1: Unsqueeze to add the head dimension
    cos = cos.unsqueeze(1)  # Shape: (B, 1, T, hs)
    sin = sin.unsqueeze(1)  # Shape: (B, 1, T, hs)

    # Step 2: Expand to match the number of heads
    cos = cos.expand(-1,x.size(1), -1, -1)  # Shape: (B, nh, T, hs)
    sin = sin.expand(-1,x.size(1), -1, -1)  # Shape: (B, nh, T, hs)
